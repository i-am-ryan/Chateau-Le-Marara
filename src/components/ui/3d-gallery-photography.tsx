...
export default function InfiniteGallery({ images, className = 'h-96 w-full', style, speed = 1, visibleCount = 12 }: {
  images: ImageItem[];
  className?: string;
    ...
    return (
      <div className={className} style={style}>
        <div className="grid grid-cols-3 gap-2 h-full overflow-hidden">
          {(images as ImageItem[]).slice(0, 6).map((img, i) => (
            <img key={i} src={typeof img === 'string' ? img : img.src} alt="" className="w-full h-full object-cover" />
          ))}
        </div>
      </div>
    );
